...
export class GoogleOAuthProvider implements OAuthHandler {
  getLoginUrl(clientType: ClientType, clientId: string, redirectUri: string): string {
    return `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&redirect_uri=${redirectUri}&response_type=code&scope=openid%20profile%20email&state=${clientType}`;
  }
